    ...
    <tr>
        <td class="br" bgcolor="#DEDEE2" bordercolor="#bfbfbf">
            <div class="photo-items-list photo-photo-list"
                id="photo_list_<?= htmlspecialcharsEx($arParams["~UNIQUE_COMPONENT_ID"])?>">
				...
				foreach ($arResult["ELEMENTS_LIST"] as $key => $arItem)
				{
					if (($arParams["SHOW_DATE"] ?? '') == "Y")
					{
						...
						if ($this_date != $current_date)
						{
							...
							?>
							<div class="group-by-days photo-date">
								<?= PhotoDateFormat($arParams["NEW_DATE_TIME_FORMAT"], MakeTimeStamp($this_date, "DD.MM.YYYY"))?>
							</div>
							<?
						}
					}
					...
					?>
					<div id="photo_cont_<?=$arItem["ID"]?>" class="photo-item-cont <?if ($arParams[" PERMISSION"]>=
						"X"){echo ' photo-item-cont-moder';}?>" title="<?= $arItem["TITLE"]?>">
						<table border="0" cellspacing="0" cellpadding="10">
							<tr>
								<td bgcolor="#333333" align="center" valign="top">
									<a class="photo-item-inner"
										style="width: <?= $arParams['THUMBNAIL_SIZE']?>px; height: <?= $arParams['THUMBNAIL_SIZE']?>px;"
										href="<?=$arItem["URL"]?>" id="photo_<?=$arItem["ID"]?>">
										<img src="<?= $src?>" border="0"
											style="<?= $item_w?> <?= $item_h?> <?= $item_left?> <?= $item_top?>;"
											alt="<?= $alt?>" />
										<?if($bNotActive ?? null):?>
										<img class="bxph-warn-icon"
											src="/bitrix/components/bitrix/photogallery.detail.list.ex/templates/.default/images/not-approved.png" />
										<?if ($arParams["PERMISSION"] >= "X" && false /* TODO : add buttons for fast approving a deleting for moderators*/):?>
										<span class="bxph-warn-link"
											style="top: <?= (round($arParams['THUMBNAIL_SIZE'] / 2) - 25)?>px; width: <?= $arParams['THUMBNAIL_SIZE']?>px;"><?= GetMessage("P_ACTIVATE")?></span>
										<span class="bxph-warn-link"
											style="top: <?= (round($arParams['THUMBNAIL_SIZE'] / 2) + 1)?>px; width: <?= $arParams['THUMBNAIL_SIZE']?>px"><?= GetMessage("P_DELETE")?></span>
										<?endif;?>
										<?endif;?>
									</a>
								</td>
			...
			<div class="empty-clear"></div>
				<?if ($arResult["MORE_PHOTO_NAV"] == "Y"):?>
					<div id="photo-more-photo-link-cont-<?= $ucid?>" class="photo-show-more">
						<img class="show-more-wait"
							src="/bitrix/components/bitrix/photogallery.detail.list.ex/templates/.default/images/wait.gif" />
							<a id="photo-more-photo-link-<?= $ucid?>" href="javascript:void(0);"
								title="<?= GetMessage("P_SLIDER_MORE_PHOTOS_TITLE")?>"><?= GetMessage("P_SLIDER_MORE_PHOTOS")?>
							</a>
					</div>
				<?endif;?>
			</div>
